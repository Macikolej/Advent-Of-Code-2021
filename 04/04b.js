...
const checkSequenceMoveNumber = (array) => {
  ...
  for (let i = 0; i < sequence.length; i ++) {
    ...
    if (i >= 4) {  // first win can only appear after 5 moves
      if (checkHitTable(hitTable) && isUnchecked) {
        isUnchecked = false;
        if (i >= loser.numberOfMoves) {
          loser.numberOfMoves = i;
          loser.table = array;
          loser.winningNumber = sequence[i];
        }
      }
    }
  }
...
console.log(sumUnmarkedNumbers(loser.table, loser.numberOfMoves) * loser.winningNumber);
